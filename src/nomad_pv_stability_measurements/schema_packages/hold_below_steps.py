"""The steps that keep one value under a bound, or between two, one class per quantity
(Design.md §17.5, §22).
...
class HoldBelowAbsoluteHumidity(HoldBelowStep):
...
class HoldBelowRelativeHumidity(HoldBelowStep):
...
class HoldBelowOxygenFraction(HoldBelowStep):
...
class HoldBetweenIrradiance(HoldBetweenStep):
