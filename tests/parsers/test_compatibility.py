"""The authored format stays readable (Design.md §13.1a, §15.2).

Every authored input the schema accepted before §13 is kept here, each with the meaning
it must keep, read from the loaded sections' own fields. `load` is the one place that
says how an authored dict reaches the schema.
"""

import os

import pytest
import yaml
from nomad.datamodel import EntryArchive, EntryMetadata
from nomad.units import ureg

from nomad_pv_stability_measurements.parsers.translate import (
    translate,
    translate_section,
)
from nomad_pv_stability_measurements.schema_packages.general import (
    CountingRepeatingBlock,
)
from nomad_pv_stability_measurements.schema_packages.hold_instructions import (
    BalanceGas,
    HoldAbsoluteHumidity,
    HoldBendRadius,
    HoldCurrent,
    HoldIrradiance,
    HoldOxygenFraction,
    HoldPressure,
    HoldRelativeHumidity,
    HoldResistance,
    HoldStrain,
    HoldTemperature,
    HoldVoltage,
)
from nomad_pv_stability_measurements.schema_packages.mpp_instructions import (
    MPPTracking,
    VOCTracking,
)
from nomad_pv_stability_measurements.schema_packages.protocol import StabilityProtocol
from nomad_pv_stability_measurements.schema_packages.ramp_instructions import (
    RampTemperature,
)

DATA_DIR = os.path.join(os.path.dirname(__file__), '..', 'data')
IRRADIANCE = ureg.watt / ureg.meter**2
BLOCK = CountingRepeatingBlock


def normalize_protocol(protocol, name, normalized, log):
    """Every instruction first, then the protocol, whose own normalize needs an archive
    with metadata."""
    for instruction in protocol.instructions:
        normalized(instruction)
    metadata = EntryMetadata(entry_name=name)
    protocol.normalize(EntryArchive(metadata=metadata, data=protocol), log)
    return protocol


@pytest.fixture
def load(normalized, log):
    """Authored instructions, translated and loaded into a protocol, normalized; the
    instructions they became. What went wrong is in the `log` fixture: the translator's
    problems and the schema's reports alike."""

    def run(*authored):
        translation = translate_section({'commands': list(authored)}, StabilityProtocol)
        log.errors.extend(problem.message for problem in translation.problems)
        protocol = StabilityProtocol.m_from_dict(translation.archive)
        return normalize_protocol(protocol, 'protocol', normalized, log).instructions

    return run


@pytest.fixture
def load_file(normalized, log):
    """A whole authored file, translated, loaded and normalized."""

    def run(name: str):
        with open(os.path.join(DATA_DIR, name)) as file:
            translation = translate(yaml.safe_load(file))
        log.errors.extend(problem.message for problem in translation.problems)
        protocol = StabilityProtocol.m_from_dict(translation.archive['data'])
        return normalize_protocol(protocol, name, normalized, log)

    return run


def magnitude(value, unit):
    return value.to(unit).magnitude


def test_the_channels_file(load_file, log):
    *settings, soak = load_file('channels.stability.yaml').instructions

    # Every word the example writes now has a place in the schema (§15.13).
    assert log.errors == []

    # Settings: instructions that never finish, holding for the whole run — a unit-ful value, the sun, and a load
    # logged as a whole, one instruction per variable.
    temperature, irradiance, *load = settings
    assert temperature.estimated_duration is None
    assert isinstance(temperature, HoldTemperature)
    assert (temperature.control, temperature.monitor) == (True, True)
    assert magnitude(temperature.set_point, ureg.degC) == pytest.approx(65)
    assert magnitude(temperature.sample_every, ureg.s) == pytest.approx(60)
    assert magnitude(irradiance.set_point, IRRADIANCE) == pytest.approx(1000)
    assert irradiance.spectrum == 'AM1.5G'
    assert [type(instruction) for instruction in load] == [
        HoldVoltage,
        HoldCurrent,
        HoldResistance,
    ]
    for instruction in load:
        assert (instruction.monitor, instruction.control, instruction.set_point) == (
            True,
            None,
            None,
        )
        assert magnitude(instruction.sampling_rate, ureg.Hz) == pytest.approx(1)

    # The routine: a block beside the settings, overriding them while it runs.
    assert soak.name == 'soak'
    hot, drift, dark, held, opened = soak.sub_instructions
    assert magnitude(hot.set_point, ureg.degC) == pytest.approx(85)
    # `500 h` is hours, not Planck's constant (§6 step 2).
    assert magnitude(hot.estimated_duration, ureg.hour) == pytest.approx(500)
    assert magnitude(hot.sampling_rate, ureg.Hz) == pytest.approx(10)
    assert (type(drift), drift.control, drift.set_point) == (
        HoldTemperature,
        None,
        None,
    )
    # Deliberately dark is a value, not a missing one (D8a).
    assert magnitude(dark.set_point, IRRADIANCE) == pytest.approx(0)
    assert isinstance(held, HoldVoltage)
    assert magnitude(held.set_point, ureg.V) == pytest.approx(0.8)
    # The load left at open circuit: an instruction of its own, asking for no number (§15.13).
    assert (type(opened), opened.control) == (VOCTracking, True)
    # One pass, `repeat: 1`: it lasts as long as its instructions together,
    # 500 h + 100 h + 100 h + 24 h + 1 h (§23).
    assert magnitude(soak.estimated_duration, ureg.hour) == pytest.approx(725)


def test_the_tree_file(load_file, log):
    [root] = load_file('tree.stability.yaml').instructions

    # The tree is only its shape: each leaf is a block with nothing in it, reported.
    assert log.errors == [
        f'{name} is a block, but has no sub-instructions.' for name in 'ABCD'
    ]
    assert root.name == 'example'
    assert [block.name for block in root.sub_instructions] == ['A', 'fork', 'D']
    assert [block.name for block in root.sub_instructions[1].sub_instructions] == [
        'B',
        'C',
    ]
    assert root.sub_instructions[1].sub_instruction_execution_mode == 'parallel'
    assert all(isinstance(block, BLOCK) for block in root.sub_instructions)


def test_one_list_holds_both_kinds_of_instruction_without_m_def(load):
    temperature, phase = load(
        {'channel': 'temperature', 'hold': '65 °C'},
        {'name': 'phase', 'commands': [{'channel': 'irradiation'}]},
    )

    assert isinstance(temperature, HoldTemperature)
    assert isinstance(phase, BLOCK)
    assert isinstance(phase.sub_instructions[0], HoldIrradiance)


def test_an_explicit_m_def_and_plain_numbers_still_load(load, log):
    # What NOMAD itself wrote back before §13: qualified `m_def`, the `channel`, a
    # `hold`, and numbers in the declared unit.
    [instruction] = load(
        {
            'm_def': 'nomad_pv_stability_measurements.schema_packages.'
            'channel_commands.TemperatureChannelCommand',
            'channel': 'temperature',
            'hold': 358.15,
            'duration': 3600,
        }
    )

    assert log.errors == []
    assert isinstance(instruction, HoldTemperature)
    assert magnitude(instruction.set_point, ureg.degC) == pytest.approx(85)
    assert magnitude(instruction.estimated_duration, ureg.hour) == pytest.approx(1)


def test_a_setpoint_is_written_either_as_the_variable_or_as_hold(load, log):
    for authored in [
        {'channel': 'mechanical', 'strain': '2 %'},
        {'channel': 'mechanical', 'variable': 'strain', 'hold': '2 %'},
    ]:
        [instruction] = load(authored)

        assert isinstance(instruction, HoldStrain)
        assert instruction.set_point.magnitude == pytest.approx(0.02)
    assert log.errors == []


def test_a_single_variable_needs_no_variable_named(load, log):
    [instruction] = load({'channel': 'mechanical', 'bend_radius': '5 mm'})

    assert isinstance(instruction, HoldBendRadius)
    assert magnitude(instruction.set_point, ureg.mm) == pytest.approx(5)
    assert log.errors == []


def test_the_isos_l_2_form(load, log):
    [instruction] = load(
        {
            'channel': 'electrical_load',
            'variable': 'voltage',
            'hold': '0.8 V',
            'sampling_rate': '10 Hz',
        }
    )

    assert log.errors == []
    assert (type(instruction), instruction.control) == (HoldVoltage, True)
    assert magnitude(instruction.set_point, ureg.V) == pytest.approx(0.8)
    assert magnitude(instruction.sampling_rate, ureg.Hz) == pytest.approx(10)
    assert magnitude(instruction.sample_every, ureg.s) == pytest.approx(0.1)


def test_open_circuit_loads_as_the_instruction_that_sits_there(load, log):
    # The word the old format wrote still reads, and now reaches an instruction (§15.13).
    for authored in [{'hold': 'open_circuit'}, {'open_circuit': True}]:
        [instruction] = load({'channel': 'electrical_load', **authored})

        assert (type(instruction), instruction.control) == (VOCTracking, True)
    assert log.errors == []


def test_mpp_loads_as_the_tracking_instruction(load, log):
    [instruction] = load(
        {'channel': 'electrical_load', 'hold': 'mpp', 'duration': '24 h'}
    )

    assert isinstance(instruction, MPPTracking)
    assert magnitude(instruction.estimated_duration, ureg.hour) == pytest.approx(24)
    assert log.errors == []


def test_a_ramp_loads_as_the_ramping_instruction(load, log):
    [instruction] = load(
        {
            'channel': 'temperature',
            'ramp': {'from': '25 °C', 'to': '85 °C', 'rate': '2 K/min'},
        }
    )

    assert isinstance(instruction, RampTemperature)
    assert magnitude(instruction.start_point, ureg.degC) == pytest.approx(25)
    assert magnitude(instruction.end_point, ureg.degC) == pytest.approx(85)
    # 60 K at 2 K/min is half an hour, which the schema works out for itself (§15.11).
    assert magnitude(instruction.estimated_duration, ureg.minute) == pytest.approx(30)
    assert log.errors == []


def test_a_named_setpoint_becomes_the_value_it_stands_for(load):
    [instruction] = load(
        {'channel': 'irradiation', 'hold': 'dark', 'spectrum': 'AM1.5G'}
    )

    assert isinstance(instruction, HoldIrradiance)
    assert magnitude(instruction.set_point, IRRADIANCE) == pytest.approx(0)
    assert instruction.spectrum == 'AM1.5G'


def test_blank_text_asks_nothing(load, log):
    for blank in ['', ' ', '\t', '\n']:
        [instruction] = load({'channel': 'temperature', 'hold': blank})

        assert (type(instruction), instruction.control, instruction.set_point) == (
            HoldTemperature,
            None,
            None,
        )
    assert log.errors == []


def test_overlapping_instructions_are_reported(load, log):
    load(
        {'channel': 'temperature', 'hold': '85 °C'},
        {'channel': 'temperature', 'monitor': True},
    )

    [error] = log.errors
    assert 'overlap' in error
    assert 'not merged' in error


# What the file gets wrong is reported, never raised, and the rest still loads (§7).


def test_an_unreadable_value_is_reported(load, log):
    [instruction] = load(
        {'channel': 'temperature', 'hold': '85 °C', 'duration': 'a fortnight'}
    )

    assert instruction.estimated_duration is None
    assert magnitude(instruction.set_point, ureg.degC) == pytest.approx(85)
    [error] = log.errors
    assert 'duration' in error


def test_a_bare_number_written_as_text_is_ambiguous(load, log):
    [instruction] = load({'channel': 'temperature', 'duration': '500'})

    assert instruction.estimated_duration is None
    assert 'bare number' in log.errors[0]


def test_a_wrong_dimension_is_reported(load, log):
    [instruction] = load({'channel': 'temperature', 'duration': '10 Hz'})

    assert instruction.estimated_duration is None
    assert len(log.errors) == 1


def test_a_named_setpoint_belongs_to_one_instruction(load, log):
    [instruction] = load({'channel': 'temperature', 'hold': 'dark'})

    assert instruction.set_point is None
    [error] = log.errors
    assert 'hold' in error


def test_a_named_setpoint_belongs_to_the_setpoint(load, log):
    [instruction] = load({'channel': 'irradiation', 'duration': 'dark'})

    assert instruction.estimated_duration is None
    assert 'not a number followed by a unit' in log.errors[0]


def test_a_hold_with_no_variable_to_go_to_is_reported(load, log):
    assert load({'channel': 'electrical_load', 'hold': '0.8 V'}) == []
    [error] = log.errors
    assert 'voltage, current, resistance' in error


def test_a_variable_the_channel_does_not_have_is_reported(load, log):
    [instruction] = load(
        {'channel': 'mechanical', 'variable': 'voltage', 'strain': '2 %'}
    )

    # Reported, never repaired: the set point stands as authored.
    assert instruction.set_point.magnitude == pytest.approx(0.02)
    [error] = log.errors
    assert 'bend_radius, strain' in error


# The atmosphere, written as an absolute volume ratio (§15.7).


def test_the_atmosphere_channel_logs_all_of_its_variables(load, log):
    logged = load({'channel': 'atmosphere', 'monitor': True})

    assert [type(instruction) for instruction in logged] == [
        HoldAbsoluteHumidity,
        HoldRelativeHumidity,
        HoldOxygenFraction,
        HoldPressure,
        BalanceGas,
    ]
    assert all(instruction.monitor for instruction in logged)
    assert log.errors == []


def test_the_atmosphere_takes_a_pressure_and_a_balance_gas(load, log):
    [pressure] = load({'channel': 'atmosphere', 'pressure': '1013.25 mbar'})
    [balance] = load({'channel': 'atmosphere', 'balance_gas': 'N2'})

    assert magnitude(pressure.set_point, ureg.pascal) == pytest.approx(101325)
    # The gas is a name, so it lands in `gas` — there is no set point to put it in.
    assert balance.gas == 'N2'
    assert 'set_point' not in BalanceGas.m_def.all_quantities
    assert log.errors == []


def test_a_gas_does_not_ramp(load, log):
    assert load({'channel': 'atmosphere', 'balance_gas': 'N2', 'ramp': {}}) == []

    [error] = log.errors
    assert 'does not ramp' in error


def test_a_glovebox_figure_and_a_volume_percent_share_one_axis(load, log):
    [glovebox] = load({'channel': 'atmosphere', 'oxygen': '0.1 ppm'})
    [ambient] = load({'channel': 'atmosphere', 'oxygen': '21 vol%'})

    assert (type(glovebox), glovebox.control) == (HoldOxygenFraction, True)
    assert glovebox.set_point.magnitude == pytest.approx(1e-7)
    assert ambient.set_point.magnitude == pytest.approx(0.21)
    assert log.errors == []


def test_water_vapour_is_written_either_as_the_variable_or_as_hold(load, log):
    for authored in [
        {'channel': 'atmosphere', 'water_vapor': '500 ppm'},
        {'channel': 'atmosphere', 'variable': 'water_vapor', 'hold': '500 ppm'},
    ]:
        [instruction] = load(authored)

        assert isinstance(instruction, HoldAbsoluteHumidity)
        assert instruction.set_point.magnitude == pytest.approx(5e-4)
    assert log.errors == []


def test_humidity_is_reported_with_what_to_write_instead(load, log):
    # `85 %RH` says nothing without the temperature it was measured at, so it is never
    # translated into a ratio: the instruction is left out, as `open_circuit` is (§15.2).
    for authored in [
        {'channel': 'atmosphere', 'humidity': '85 %RH'},
        {'channel': 'atmosphere', 'variable': 'humidity', 'hold': '85 %RH'},
    ]:
        assert load(authored) == []
    # Humidity is two variables now, and the message names both (§20.6).
    assert ['relative_humidity' in error for error in log.errors] == [True, True]
    assert ['absolute_humidity' in error for error in log.errors] == [True, True]


def test_a_relative_humidity_loads_as_the_ratio_it_is(load, log):
    # ISOS Table 1 prints `85%` at 65 °C; the archive keeps the absolute ratio (§15.16).
    [instruction] = load(
        {'channel': 'atmosphere', 'water_vapor': {'rh': '85 %', 'at': '65 °C'}}
    )

    assert isinstance(instruction, HoldAbsoluteHumidity)
    assert instruction.set_point.magnitude == pytest.approx(0.2109, rel=1e-3)
    assert log.errors == []


def test_a_relative_humidity_value_on_the_water_axis_is_refused(load, log):
    [instruction] = load({'channel': 'atmosphere', 'water_vapor': '85 %RH'})

    # Reported, never repaired: the instruction stays, without a set point nobody can read.
    assert (isinstance(instruction, HoldAbsoluteHumidity), instruction.set_point) == (
        True,
        None,
    )
    [error] = log.errors
    assert 'not a volume ratio' in error
