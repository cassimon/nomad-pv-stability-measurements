...
HOLDS = HoldInstruction.__subclasses__()
#: The instruction that holds something that is no number: a gas. The load's tracked point is
#: the same shape, in `mpp_instructions.py` (§15.10).
...
def test_every_instruction_has_a_monitor_and_a_control_tag_and_a_setpoint():
    for cls in HOLDS:
...
def test_each_setpoint_uses_nomads_unit_system():
    ...
    assert HoldInstruction.m_def.all_quantities['set_point'].unit is None


def test_a_setpoint_reloads_in_its_unit():
    data = {'control': True, 'set_point': 338.15, 'estimated_duration': 3600.0}
    instruction = HoldTemperature.m_from_dict(data)

    assert instruction.set_point.to(ureg.degC).magnitude == pytest.approx(65)
...
def test_a_physical_property_is_a_plain_field_of_its_instruction():
...
def test_the_schema_holds_no_physics():
    ...
    for cls in [MonitorControlInstruction, *HOLDS, *NUMBERLESS]:
...
def test_an_atmosphere_instruction_round_trips_as_the_fraction():
    data = {'monitor': True, 'control': True, 'set_point': 5e-4}

    assert HoldAbsoluteHumidity.m_from_dict(data).m_to_dict() == data


def test_no_atmosphere_instruction_knows_relative_humidity():
    # It is a property of the water content and the temperature together, so it is no
    # field of either instruction; a `normalize()` can derive it once something needs it.
...
# The instructions that hold no number, and the ambient quantities (§15.10, §15.11).


def test_an_instruction_that_holds_no_number_takes_no_setpoint():
    # With `set_point` on `HoldInstruction` rather than on the base, these two inherit no
    # numeric field they would never fill (§15.11).
    for cls in NUMBERLESS:
        assert not issubclass(cls, HoldInstruction)
...
def test_a_temperature_tolerance_is_a_difference_in_kelvin():
    ...
    assert instruction.set_point_tolerance.to(ureg.kelvin).magnitude == pytest.approx(2)
...
def test_the_base_takes_any_point_and_a_child_narrows_it():
    assert set(MonitorControlInstruction.m_def.all_quantities) >= {'reference_point'}
